...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔍 Obteniendo tareas del estudiante (método simplificado)...')
    ...
    // Para cada curso, obtener las entregas del estudiante
    for (const course of courses) {
      try {
        ...
        // Para cada tarea, obtener la entrega del estudiante
        for (const work of coursework) {
          try {
            ...
            // Silenciar errores de entregas individuales para no saturar los logs
            // console.warn(`⚠️ Error obteniendo entrega para tarea ${work.title}:`, submissionError)
          }
        }
      } catch (courseError: any) {
        console.warn(`⚠️ Error procesando curso ${course.name}:`, courseError.message)
        // Si es un error 403, significa que no tenemos permisos para este curso
        if (courseError.code !== 403) {
          // Solo loggear errores que no sean de permisos
